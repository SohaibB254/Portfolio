...
const Services = () => {
  ...
  useFadeUpOnScroll('.service-card')
    let services = [
      ...
      <div className='flex flex-col gap-8 lg:gap-24 '>
        {/* Service */}
        {
            services.map((s,idx) =>{
                let isOdd = s.id%2 === 0;
                return (
                     <div key={idx}  className={`flex service-card   ${isOdd ?'flex-row-reverse ':''} lg:gap-24 gap-8 justify-between items-center lg:items-start `}>
        <div className='lg:w-[350px] w-[250px] h-[200px]  lg:h-[300px] object-cover   hidden md:block overflow-hidden rounded-md ' id="image">
            <img className='w-full h-full hover:grayscale-0 transition-all duration-500 grayscale-100' src={s.img} alt="" />
        </div>
